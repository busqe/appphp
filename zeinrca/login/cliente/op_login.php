...
if($_GET["modo"]=="passaporte" && strlen($_GET["us"])>0){
	...
	$mensagem .= "Acessando pelo usuario <br />";
...
if($acao_acesso=="entrar"){
	...
	$mensagem .=  "Conectado... <br />";
 	$sq_us = mysql_query("SELECT * FROM  tab_colaborador WHERE usoMail = '".$login_mail."' AND usoPas = '".$login_passw."' AND usoPub = 1 LIMIT 1", $banco02);
 	...
 	if(mysql_num_rows($sq_us) > 0)
	{
		   ...
		   $mensagem .=  "Listado usuario... <br />";
 			 //$_SESSION['uso_login_session'] = $login_mail;  
			 //$_SESSION['uso_senha_session'] = $login_passw;
