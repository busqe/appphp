...
if($_GET["modo"]=="passaporte" && strlen($_GET["us"])>0){
			...
			$codigo_us = $_GET["us"];
			 ...
			 $rs_entrar = mysql_query("SELECT * FROM  tab_usuario WHERE usoCodUso=".$codigo_us." LIMIT 1", $banco02);
			 if(mysql_num_rows($rs_entrar)>0)
			 {
					 $linha_us = mysql_fetch_array($rs_entrar);
					 $login_mail =			trim($linha_us[usoMail]);
					 $login_passw =			trim($linha_us[usoPas]);
...
if($acao_acesso=="entrar"){
	...
	$mensagem .=  "Conectado... <br />";
 	$sq_us = mysql_query("SELECT 
 		...
 		usoMail as usEmail,
 		usoPas as usSenha
 		  FROM  tab_usuario WHERE usoMail = '".$login_mail."' AND usoPas = '".$login_passw."' AND usoPub = 1 LIMIT 1", $banco02);
 	...
 	if(mysql_num_rows($sq_us) > 0)
	{
 		...
 		*/
			...
			$sqB = "UPDATE tab_usuario  SET usuOnline=1 WHERE usoCodUso = ".$usoCodUso." ";  
...
if($acao_acesso=="sair"){
			...
			if(strlen($usuario_id)>1){
				     ...
				     $mensagem .=  "Selecionando Usuario...<br />";				
					 $sq_usuario = mysql_query("SELECT * FROM  tab_usuario WHERE usoCodUso=".$usuario_id." LIMIT 1", $banco02);
					 if(mysql_num_rows($sq_usuario)>0)
					 {
							 $sq_sair = "UPDATE tab_usuario  SET usuOnline=0 WHERE usoCodUso=".$usuario_id."";  
