...
if($banco01){
	...
	if($usuario > 0){ 
		...
		$mensagem .=  "Conectando... <br />";
 		...
 		$sq_us = "SELECT *
 		FROM  TAB_PEDIDO_APLICATIVO WHERE CODLINK=".$link." 
	 	...
	 	if($total > 0)
		{
	        ...
	        if($usuario>0 && $banco00){
				...
				$sq_carrinho = "SELECT * FROM TAB_LINK_CODIGO WHERE 
...
\"publicar\": \"".$PUBLICAR."\",\"sortimento\": \"".$SORTIMENTO."\",\"quantidade\": \"".$QUANTIDADE."\",\"usr\": \"".$usuario."\" }".$virgula."\n"; 
